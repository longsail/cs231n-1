...
def init_two_layer_conv3net(weight_scale=1e-3, bias_scale=0, input_shape=(1, 16, 16, 16),
  ...
  model = {}
  model['W1'] = weight_scale * np.random.randn(num_filters, C, filter_size, filter_size, filter_size)
  model['b1'] = bias_scale * np.random.randn(num_filters)
  model['W2'] = weight_scale * np.random.randn(num_filters * H * W * D / 8, num_classes)
  model['b2'] = bias_scale * np.random.randn(num_classes)
  return model


def two_layer_conv3net(X, model, y=None, reg=0.0, dropout=1.0):
  """
  ...
  conv_filter_height, conv_filter_width, conv_filter_depth = W1.shape[2:]
  ...
  assert conv_filter_depth % 2 == 1, 'Conv filter depth must be odd'
  conv_param = {'stride': 1, 'pad': (conv_filter_height - 1) / 2}
  pool_param = {'pool_height': 2, 'pool_width': 2, 'pool_depth': 2, 'stride': 2}
  dropout_param = {'p': dropout}
  dropout_param['mode'] = 'test' if y is None else 'train'

  # Compute the forward pass
  a1, cache1 = conv3_relu_pool3_forward(X, W1, b1, conv_param, pool_param)
  ...
  dX,  dW1, db1 = conv3_relu_pool3_backward(da1, cache1)
...
def init_three_layer_conv3net(input_shape=(1, 16, 16, 16), num_classes=10,
                            filter_size=3, num_filters=(32, 64),
  ...
  F1, FC = num_filters
  model = {}
  model['W1'] = np.random.randn(F1, C, filter_size, filter_size, filter_size)
  model['b1'] = np.random.randn(F1)
  model['W2'] = np.random.randn(H * W * D * F1 / 8, FC)
...
def three_layer_conv3net(X, model, y=None, reg=0.0, dropout=None):
  """
  ...
  pool_param = {'stride': 2, 'pool_height': 2, 'pool_width': 2, 'pool_depth': 2}
  dropout_param = {'p': dropout}
  dropout_param['mode'] = 'test' if y is None else 'train'
...
def init_simple_net(weight_scale=1e-3, bias_scale=0, input_shape=(1, 16, 16, 16),
                           num_classes=10):
  """
  Initialize the weights for a two-layer ConvNet.
  ...
  model = {}
  model['W1'] = weight_scale * np.random.randn(C*H*W*D, num_classes)
  model['b1'] = bias_scale * np.random.randn(num_classes)
  return model


def simple_net(X, model, y=None, reg=0.0, dropout=1.0):
  """
  Compute the loss and gradient for a simple two-layer ConvNet. The architecture
  is conv-relu-pool-affine-softmax, where the conv layer uses stride-1 "same"
  convolutions to preserve the input size; the pool layer uses non-overlapping
  ...
  # a1, cache1 = conv3_relu_pool3_forward(X, W1, b1, conv_param, pool_param)
  # d1, cache2 = dropout_forward(a1, dropout_param)
  scores, cache1 = affine_forward(X, W1, b1)
  ...
  dW1 += reg * W1
  # dW2 += reg * W2
  reg_loss = 0.5 * reg * np.sum(np.sum(W1 * W1))
