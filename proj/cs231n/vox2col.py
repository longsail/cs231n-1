...
def col2vox_indices(cols, x_shape, field_height=3, field_width=3, field_depth=3,
                    padding=1, stride=1):
  """ An implementation of col2im based on fancy indexing and np.add.at """
  N, C, H, W, D = x_shape
  H_padded, W_padded, D_padded = H + 2 * padding, W + 2 * padding, D + 2 * padding
  x_padded = np.zeros((N, C, H_padded, W_padded, D_padded), dtype=cols.dtype)
  l, i, j, k = get_vox2col_indices(x_shape, field_height, field_width, field_depth, 
                                  padding, stride)
  cols_reshaped = cols.reshape(C * field_height * field_width * field_depth, -1, N)
  cols_reshaped = cols_reshaped.transpose(2, 0, 1)
